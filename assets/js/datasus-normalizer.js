...
function periodLabelFromKeys(source, timeKeys) {
  ...
  if (!labels.length) return 'sem periodo valido';
...
export function deriveCorrelationPairs({
  ...
  const errors = [];
  if (pairs.length < 3) {
    errors.push('Selecione pelo menos 3 pares validos para calcular correlacao.');
  }
...
export function derivePraisSeries({
  ...
  const errors = [];
  if (ordered.length < 5) {
    errors.push('Selecione pelo menos 5 pontos temporais validos.');
  }
...
export function suggestTestsForSources(sources) {
  ...
  sources.forEach(source => {
    ...
    if (metricCount >= 2) {
      suggestions.push(`A base ${source.fileName} possui pelo menos duas variaveis quantitativas; correlacao e uma boa candidata.`);
    }
    if (categoryCount === 2) {
      suggestions.push(`A base ${source.fileName} possui duas categorias principais; t de Student pode ser apropriado.`);
    }
    if (categoryCount >= 3) {
      suggestions.push(`A base ${source.fileName} possui 3 ou mais grupos; ANOVA pode ser considerada como proximo passo.`);
    }
    if (timeCount >= 5) {
