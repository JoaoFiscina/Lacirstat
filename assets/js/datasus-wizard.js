...
const ROLE_OPTIONS = [
  { value: 'primary-category', label: 'Dimensao principal' },
...
function linePreview(source) {
  return source.parsed.lines.slice(0, 10).map(line => [
    `Linha ${line.index + 1}`,
...
function sourceCardHtml(source, activeId, utils) {
  ...
  return `
    <button type="button" class="datasus-source-card ${source.id === activeId ? 'is-active' : ''}" data-source-id="${utils.escapeHtml(source.id)}">
      ...
      <span>${utils.escapeHtml(source.parsed.diagnosis.formatType)} · cabecalho linha ${source.parsed.headerRowIndex + 1}</span>
    </button>
  `;
}
      ...
      <tr>
        ...
        <td>
          <select data-action="column-role" data-column-index="${profile.index}">
            ${ROLE_OPTIONS.map(option => `<option value="${option.value}"${option.value === mapped.role ? ' selected' : ''}>${utils.escapeHtml(option.label)}</option>`).join('')}
          </select>
        </td>
        <td>
...
function normalizedPreviewHtml(source, utils) {
  ...
  if (!normalized?.previewRows?.length) {
    return '<div class="small-note">Ainda nao ha linhas normalizadas suficientes para pre-visualizacao.</div>';
  }

  const rows = normalized.previewRows.map(row => [
    row.category || '',
    row.time || '',
  ...
  return utils.renderPreviewTable(['Categoria', 'Tempo', 'Valor', 'Total'], rows, 12);
}
...
export function createDatasusWizard({
  ...
  function render() {
    ...
    const suggestionHtml = session.suggestions.length
      ? `
        <div class="info-banner" style="margin-top:14px;">
          <strong>Orientacao metodologica</strong>
      ...
      <div class="datasus-wizard">
        <div class="datasus-toolbar">
          ...
          <div class="datasus-toolbar-actions">
            <label class="btn-secondary datasus-upload-btn" for="datasus-upload-input">Importar arquivo(s)</label>
            <input id="datasus-upload-input" type="file" multiple accept=".csv,.txt,.tsv,text/plain,text/csv" />
            ${exampleSources.length ? '<button type="button" class="btn-ghost" data-action="load-example">Carregar exemplo</button>' : ''}
          ...
          <div class="datasus-step-grid">
            <section class="surface-card">
              <div class="tstudent-step-head">
                ...
                <h4>Confirmar a linha de cabecalho</h4>
              </div>
              ...
              <div class="form-grid two" style="margin-top:14px;">
                <div>
                  ...
                  <select id="datasus-format-select">
                    ...
                    <option value="hybrid"${source.mapping.formatType === 'hybrid' ? ' selected' : ''}>Nao tenho certeza / hibrida</option>
                  </select>
                </div>
                <div>
                  ...
                  <div class="datasus-summary-metrics">
                    ...
                    <span class="small-chip ${source.parsed.diagnosis.hasTotalColumn ? 'warning' : 'info'}">Total: ${source.parsed.diagnosis.hasTotalColumn ? 'sim' : 'nao'}</span>
                  </div>
                </div>
              </div>
            </section>
            ...
            <section class="surface-card">
              <div class="tstudent-step-head">
                ...
                <h4>Mapear os papeis das colunas</h4>
              </div>
              <p class="small-note datasus-step-note">Defina a dimensao principal, as colunas temporais, medidas, totais e o que deve ser ignorado.</p>
            ...
            <section class="surface-card">
              <div class="tstudent-step-head">
                ...
                <h4>Confirmar tipos de variavel</h4>
              </div>
            ...
            <section class="surface-card">
              <div class="tstudent-step-head">
                ...
                <h4>Pre-visualizar a base normalizada</h4>
              </div>
              <div class="metrics-grid" style="margin-top:14px;">
                <div class="metric-card">
                  ...
                  <div class="metric-mini">Validos: ${source.normalized.summary.validRecordCount}</div>
                </div>
                <div class="metric-card">
              ...
              ${source.normalized.errors.length ? `
                <div class="error-box" style="margin-top:14px;">
                  <strong>Normalizacao incompleta.</strong>
                  <ul class="datasus-inline-list">
                    ${source.normalized.errors.map(error => `<li>${utils.escapeHtml(error)}</li>`).join('')}
                  </ul>
                </div>
              ` : `
                <div class="success-box" style="margin-top:14px;">A base interna esta pronta para ser usada pelos modulos estatisticos.</div>
              `}
              <div style="margin-top:14px;">
            ...
            <section class="surface-card">
              <div class="tstudent-step-head">
                ...
                <h4>Confirmar e enviar para a analise</h4>
              </div>
              <p class="small-note datasus-step-note">A confirmacao trava esta versao da base como entrada confiavel para o modulo atual e para outros modulos que usem a ultima sessao DATASUS.</p>
              ...
              <div class="${source.confirmed ? 'success-box' : 'status-bar'}" style="margin-top:14px;">
                ${utils.escapeHtml(source.confirmed ? 'Base confirmada. Os testes ja podem consumi-la.' : 'Base ainda nao confirmada. Revise o mapeamento antes de prosseguir.')}
              </div>
            </section>
          </div>
        ` : `
          <div class="small-note" style="margin-top:16px;">Nenhum arquivo carregado ainda.</div>
        `}
      </div>
    `;

    root.querySelector('#datasus-upload-input')?.addEventListener('change', async event => {
    ...
    root.querySelector('[data-action="clear-all"]')?.addEventListener('click', () => {
      ...
      state.statusTone = 'status';
      state.statusMessage = 'Fluxo DATASUS limpo. Importe um novo arquivo para recomecar.';
      render();
      notify();
    });

    root.querySelector('[data-action="load-example"]')?.addEventListener('click', async () => {
      if (!exampleSources.length) return;
      await addTextSources(exampleSources, 'Exemplo DATASUS carregado para revisao guiada.');
    });
