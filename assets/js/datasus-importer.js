...
function scoreHeaderCandidate(row, followingRows, utils, stats) {
  ...
  if (nonEmptyCells.length < 2) {
    return {
      score: -100,
      reasons: ['Poucas celulas preenchidas.']
    };
  }
  ...
  if (keywordCount) reasons.push(`${keywordCount} termo(s) tipicos de cabecalho`);
  if (nextAlignedRows.length) reasons.push(`${nextAlignedRows.length} linha(s) seguintes com estrutura compativel`);
...
export function parseDatasusText({ text, fileName = '', utils, stats, headerRowIndex = null }) {
  ...
  if (!lines.length) {
    return {
      ok: false,
      fileName,
      error: 'Nenhum conteudo foi encontrado no arquivo informado.'
    };
  }
