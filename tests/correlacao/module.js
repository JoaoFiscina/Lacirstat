...
function buildScatterSvg(dataset, pearson, outlierFlags, utils) {
  ...
  const equationText = `${utils.escapeHtml(dataset.headers[1])} = ${utils.fmtNumber(pearson.intercept, 2)} ${equationSign} ${utils.fmtNumber(Math.abs(pearson.slope), 2)} x ${utils.escapeHtml(dataset.headers[0])}`;
  ...
  return `
    <svg viewBox="0 0 ${width} ${height}" class="scatter-svg" role="img" aria-label="Dispersão com tendência linear">
      ...
      <text x="${width - 270}" y="68" fill="#33556f" font-size="11">${equationText}</text>
...
function buildMethodInterpretation(dataset, pearson, spearman, outlierLabels, activeMethod, utils) {
  ...
  const gap = Math.abs(Math.abs(pearson.coef) - Math.abs(spearman.coef));
  if (gap > 0.15) {
    text += ` Como Pearson (${utils.fmtSigned(pearson.coef, 3)}) e Spearman (${utils.fmtSigned(spearman.coef, 3)}) diferiram de maneira perceptivel, recomenda-se interpretar com cautela.`;
  } else {
    text += ` O metodo alternativo (${methodLabel(activeMethod === 'spearman' ? 'pearson' : 'spearman')}) permaneceu consistente, com coeficiente ${utils.fmtSigned(secondary.coef, 3)}.`;
  }
  if (outlierLabels.length) {
    text += ` Foram detectados possiveis pontos extremos (${outlierLabels.slice(0, 4).join(', ')}${outlierLabels.length > 4 ? ', ...' : ''}), lembrando que Pearson tende a ser mais sensivel a outliers do que Spearman.`;
  }
...
function buildCorrelationDatasetFromTabularState(fileState, stats, sourceMeta = {}) {
  ...
  if (fileState.delimiter === ';') {
    infos.push(`${sourceKind === 'file' ? 'Arquivo' : 'Conteudo colado'} lido no padrao ponto e virgula (;).`);
  } else if (fileState.delimiter === '\t') {
    infos.push('Conteudo tabulado do Excel interpretado automaticamente.');
  }
  if (fileState.decimalCommaDetected) {
    infos.push('Numeros com virgula decimal foram convertidos automaticamente.');
  }
  infos.push('ID e apenas rotulo; variavel_x e variavel_y entram no calculo.');
  if (!recognizedColumns.id) {
    infos.push('Coluna de ID nao reconhecida; a previa usa a ordem das linhas como referencia.');
  }
  if (fileState.duplicates.length) {
    warnings.push(`Cabecalhos duplicados foram ignorados: ${fileState.duplicates.join(', ')}.`);
  }
